...
const twoType = pokemon.filter(p => p.types.length > 1);
const pNames = pokemon.map(p => p.name);
const highId = pokemon.filter(p => p.id > 99).map(p => p.name);
const poisonNames = pokemon.filter(p => p.types.length === 1 && p.types.includes("poison")).map(p => p.name);
const flyingType = pokemon.filter(p => p.types[1] === "flying").map(p => p.types[0]);
const normals = pokemon.filter(p => p.types.includes("normal")).length
