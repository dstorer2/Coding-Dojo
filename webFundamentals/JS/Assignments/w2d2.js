...
//console.log the pokémon objects whose id is evenly divisible by 3
    //modulo operator %.......finds remainder of two operands
...
function reverseName(){
    for(var j = 0; j < pokemon[i].name.length / 2; i++){
        var reversed = ""
        var temp = pokemon[i].name[j];
        pokemon[i].name[j] == pokemon[i].name[pokemon[i].name.length - 1 - j]
        pokemon[i].name[pokemon[i].name.length - 1 - j] = temp
        return reversed
    }
}
for (var i = 0; i < pokemon.length; i++){
